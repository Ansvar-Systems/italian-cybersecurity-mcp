# ─────────────────────────────────────────────────────────────────────────────
# Italian Cybersecurity MCP — multi-stage Dockerfile
# ─────────────────────────────────────────────────────────────────────────────
# Build:  docker build -t italian-cybersecurity-mcp .
# Run:    docker run --rm -p 3000:3000 italian-cybersecurity-mcp
#
# The image bakes /app/data/acn.db at build time. Override ACN_DB_PATH
...
ENV ACN_DB_PATH=/app/data/acn.db
...
# Bake the pre-built database into the image so /app/data/acn.db resolves
# at runtime without a bind mount. This matches the working pattern in
# cypriot-data-protection-mcp's GHCR image.
#
# The explicit `data/acn.db` reference is required — `.github/workflows/
...
# We then COPY it into the image at /app/data/acn.db (ACN_DB_PATH). The
# explicit `data/<name>.db` reference is required for the workflow's grep
# `COPY\s+\K(data/\S+\.db)` to match.
COPY data/database.db data/acn.db
